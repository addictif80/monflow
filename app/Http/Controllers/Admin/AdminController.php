...
class AdminController extends Controller
{
    ...
    {
        return view('admin.newsletters.list', ['newsletters' => Newsletter::latest()->paginate(20)]);
    }

    public function newsletterCreate(Request $request)
    {
        if ($request->isMethod('post')) {
            $data = $request->validate(['subject' => 'required', 'html_body' => 'required']);
            Newsletter::create($data);
            return redirect('/admin/newsletters')->with('success', 'Brouillon créé.');
        }
    ...
    public function newsletterEdit(string $id, Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            $data = $request->validate(['subject' => 'required', 'html_body' => 'required']);
            $nl->update($data);
            return redirect('/admin/newsletters')->with('success', 'Newsletter mise à jour.');
        }
    ...
    public function newsletterSend(string $id, EmailService $mail)
    {
        $nl = Newsletter::findOrFail($id);
        if ($nl->status !== 'draft') {
            return back()->with('error', 'Cette newsletter a déjà été envoyée.');
        }
        ...
        return back()->with('success', "Newsletter envoyée à {$sent} abonné(s).");
    }

    public function newsletterPreview(string $id)
    {
        $nl = Newsletter::findOrFail($id);
        $ctx = ['username' => 'aperçu', 'first_name' => 'Aperçu', 'site_name' => config('app.name'), 'site_url' => config('app.url')];
        $html = $nl->html_body;
        foreach ($ctx as $k => $v) {
            $html = str_replace(["{{ {$k} }}", "{{{$k}}}"], $v, $html);
        }
        return $html;
    }
